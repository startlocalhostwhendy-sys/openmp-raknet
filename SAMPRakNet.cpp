...
// 0 = disabled (checks in ReliabilityLayer guard on > 0)
unsigned int SAMPRakNet::messagesLimit_ = 0;
unsigned int SAMPRakNet::messageHoleLimit_ = 0;
unsigned int SAMPRakNet::acksLimit_ = 0;
...
bool SAMPRakNet::verboseNetworkLogs_ = false;
bool SAMPRakNet::tempConnectionProtection_ = false;
    ...
    Decrypt(uint8_t const* src, int len)
{
	...
	static uint8_t inverseTable[256];
	static bool inverseBuilt = false;
	if (!inverseBuilt) {
		for (int i = 0; i < 256; ++i) {
			inverseTable[sampEncrTable[i]] = (uint8_t)i;
		}
		inverseBuilt = true;
	}
	...
	for (int i = 0; i < payloadLen; ++i) {
		const uint8_t mask = (i & 1) == 0
			? (uint8_t)(((port ^ 0x5555) >> 8) & 0xFF)
			: (uint8_t)((port ^ 0x55) & 0xFF);
		const uint8_t encrypted = (uint8_t)src[i + 1] ^ mask;
		const uint8_t plain = inverseTable[encrypted];
		checksum ^= plain & 0x99;
...
bool
SAMPRakNet::
	ShouldLogSyncOrRPCPacket(uint8_t packetId)
{
	switch (packetId)
	{
		// Service packets useful to diagnose reconnects/disconnects.
		case 36: // ID_OPEN_CONNECTION_REQUEST
		case 37: // ID_CONNECTION_REQUEST
		case 38: // ID_OPEN_CONNECTION_REPLY
		case 17: // ID_CONNECTION_LOST
		case 18: // ID_DISCONNECTION_NOTIFICATION
		case 30: // ID_CONNECTED_PONG
		case 31: // ID_PING
		case 32: // ID_INTERNAL_PING
		case 34: // ID_DETECT_LOST_CONNECTIONS
			return true;
...
)
{
	...
	else
	{
		...
		{
			// Compatibility: do not suppress repeated open-connection probes from the same IP.
			// Clients may retry from a new UDP source port before the previous attempt finalizes.
			// Returning true here allows ID_OPEN_CONNECTION_REPLY to be sent immediately.
			return true;
		}
		else
		{
