...
int SocketLayer::RecvFrom( const SOCKET s, RakPeer *rakPeer, int *errorCode )
{
	...
	if ( len != SOCKET_ERROR )
	{
		...
		if (decrypted) {
			auto* const core = SAMPRakNet::GetCore();
			if (core != nullptr)
			{
				...
				for (int idx = 0; idx < len - 1; ++idx)
				{
					if (idx != 0)
						bytesStream << ' ';
					bytesStream << std::setw(2) << static_cast<unsigned int>(decrypted[idx]);
				}

				core->printLn(
					"[NET-DECRYPT] id=%u len=%d bytes=%s",
					static_cast<unsigned int>(decrypted[0]),
					len - 1,
					bytesStream.str().c_str());
			}
...
int SocketLayer::SendTo( SOCKET s, const char *data, int length, unsigned int binaryAddress, unsigned short port )
{
	...
	do
	{
		// Outgoing packets are sent as plain payload for compatibility with custom clients.
