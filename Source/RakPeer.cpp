...
namespace RakNet
{
	...
	static const RpcIdRemapEntry g_rpcIdRemap[] =
	{
		...
		{136,502},{62,514},{41,369},{42,512},{23,515},{60,343},{83,436},{119,317},{115,389},{27,408},
		{117,408},{68,349},{69,396},{153,305},{11,437},{12,392},{13,341},{14,346},{70,318},{71,362},
		{72,396},{73,427},{156,368},{157,307},{158,357},{159,435},{160,325},{161,311},{162,356},{15,301},
		{16,369},{17,347},{18,322},{19,352},{20,398},{21,334},{22,402},{65,312},{66,342},{55,412},
		{56,320},{144,666},{145,350},{146,424},{147,373},{148,298},{149,337},{44,401},{45,323},{46,326},
		{47,335},{79,434},{80,434},{99,310},{122,433},{123,509},{124,422},{126,416},{127,297},{57,419},
		{74,366},{75,511},{76,343},{77,343},{78,387},{133,400},{134,378},{135,363},{105,302},{108,358},
		{120,332},{121,374},{85,385},{86,319},{87,306},{88,309},{104,424},{89,430},{90,343},{91,345},
		{35,431},{61,348},{36,327}
	};
	static const unsigned int g_rpcIdRemapCount = sizeof(g_rpcIdRemap) / sizeof(g_rpcIdRemap[0]);

	static unsigned short MapOutgoingLegacyRpcToWire(unsigned char legacy8)
	{
		for (unsigned int i = 0; i < g_rpcIdRemapCount; ++i)
		{
			if (g_rpcIdRemap[i].legacy8 == legacy8)
				return g_rpcIdRemap[i].wire9;
		}
		return (unsigned short)legacy8;
	}

	static bool FindIncomingWireCandidates(unsigned short wire9, unsigned char* outLegacy, unsigned int maxOut, unsigned int* outCount)
	{
		unsigned int count = 0;
		for (unsigned int i = 0; i < g_rpcIdRemapCount; ++i)
		{
			if (g_rpcIdRemap[i].wire9 == wire9)
			{
				if (count < maxOut)
					outLegacy[count] = g_rpcIdRemap[i].legacy8;
				++count;
			}
		}
		if (outCount)
...
static const char g_bridgeRpcMarker[4] = { 'B', 'A', 'Z', 'E' };
static const unsigned char g_bridgeRpcFormatVersion = 1;
// Bridge delivery mode:
// g_bridgeMirrorOriginalRpc=true keeps standard RakNet RPC path active.
static const bool g_bridgeMirrorOriginalRpc = false;

	struct CompatRpcBitWriter
	{
		std::vector<unsigned char> out;
		unsigned int bitOffset;

		CompatRpcBitWriter() : bitOffset(0) {}

		static inline unsigned int BitsToBytes(unsigned int bits) { return (bits + 7u) >> 3; }

		void EnsureBits(unsigned int addBits)
		{
			const unsigned int needed = BitsToBytes(bitOffset + addBits);
			if (needed > out.size())
				out.resize(needed, 0);
		}

		// Mirrors plugin.cpp BitWriter::writeBits behavior.
		void WriteBitsCompat(const unsigned char* input, int numberOfBitsToWrite, bool rightAlignedBits)
		{
			if (!input || numberOfBitsToWrite <= 0)
				return;

			EnsureBits((unsigned int)numberOfBitsToWrite);

			int offset = 0;
			while (numberOfBitsToWrite > 0)
			{
				unsigned char dataByte = *(input + offset);
				const int numberOfBitsUsedMod8 = (int)(bitOffset & 7u);
				if (numberOfBitsToWrite < 8 && rightAlignedBits)
					dataByte <<= (8 - numberOfBitsToWrite);

				if (numberOfBitsUsedMod8 == 0)
					out[bitOffset >> 3] = dataByte;
				else
				{
					out[bitOffset >> 3] |= (unsigned char)(dataByte >> numberOfBitsUsedMod8);
					if (8 - numberOfBitsUsedMod8 < 8 && 8 - numberOfBitsUsedMod8 < numberOfBitsToWrite)
						out[(bitOffset >> 3) + 1] = (unsigned char)(dataByte << (8 - numberOfBitsUsedMod8));
				}

				if (numberOfBitsToWrite >= 8)
					bitOffset += 8;
				else
					bitOffset += (unsigned int)numberOfBitsToWrite;
		...
		// Mirrors plugin.cpp BitWriter::writeCompressedUInt32 behavior.
		void WriteCompressedUInt32Compat(unsigned int value)
		{
			unsigned char input[4];
			input[0] = (unsigned char)(value & 0xFFu);
			input[1] = (unsigned char)((value >> 8) & 0xFFu);
			input[2] = (unsigned char)((value >> 16) & 0xFFu);
			input[3] = (unsigned char)((value >> 24) & 0xFFu);

			int currentByte = 3;
			while (currentByte > 0)
			{
				if (input[currentByte] == 0)
				{
					const unsigned char one = 1;
					WriteBitsCompat(&one, 1, true);
				}
				else
				{
					const unsigned char zero = 0;
					WriteBitsCompat(&zero, 1, true);
					WriteBitsCompat(input, (currentByte + 1) * 8, true);
					return;
				}
				--currentByte;
			}
			...
			if ((input[0] & 0xF0u) == 0)
			{
				const unsigned char one = 1;
				WriteBitsCompat(&one, 1, true);
				WriteBitsCompat(input, 4, true);
			}
			else
			{
				const unsigned char zero = 0;
				WriteBitsCompat(&zero, 1, true);
				WriteBitsCompat(input, 8, true);
			}
		}

		void Finalize()
		{
			out.resize(BitsToBytes(bitOffset));
		}
	};

	static bool BuildCompatIdRpcPacket(unsigned short rpcId9, const unsigned char* payload, unsigned int payloadBits, std::vector<unsigned char>& outPacket)
	{
		CompatRpcBitWriter bw;
		const unsigned char packetId = ID_RPC;
		bw.WriteBitsCompat(&packetId, 8, true);
		bw.WriteBitsCompat((const unsigned char*)&rpcId9, 9, true);
		bw.WriteCompressedUInt32Compat(payloadBits);
		if (payloadBits > 0 && payload)
			bw.WriteBitsCompat(payload, (int)payloadBits, false);
		bw.Finalize();
		outPacket.swap(bw.out);
		return true;
	}
...
static const RakNetTime LEGACY_HANDSHAKE_TIMEOUT_MS = 30000;
// Legacy compatibility mode:
// keep already connected players online even when ACKs stop arriving.
static const bool LEGACY_KEEP_CONNECTED_WITHOUT_ACK = true;
...
//	char *writeTarget; // Used to hold either a block of allocated data or the externally allocated data
	...
	for (sendListIndex=0; sendListIndex < (unsigned)sendListSize; sendListIndex++)
	{
		...
		unsigned short bridgeRpcWireId = 0;
		unsigned char bridgeRpcLegacyId = 0xFF;
		...
		bridgeRpcWireId = rpcIndex == UNDEFINED_RPC_INDEX ? 0xFFFF : (unsigned short)rpcIndex;
#else
		// SA-MP client compatibility: RPC ID is serialized on 9 bits on the wire.
		unsigned short rpcId9 = MapOutgoingLegacyRpcToWire((unsigned char)uniqueID);
		outgoingBitStream.WriteBits((const unsigned char*)&rpcId9, 9, true);
		bridgeRpcWireId = rpcId9;
		bridgeRpcLegacyId = (unsigned char)uniqueID;
#endif
	#if !RAKNET_LEGACY
			outgoingBitStream.Write((bool) ((replyFromTarget!=0)==true));
	#endif

			if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
			{
			#if RPCID_STRING
				const int rpcLogId = rpcIndex == UNDEFINED_RPC_INDEX ? -1 : (int)rpcIndex;
			#else
				const int rpcLogId = (int)(unsigned char)uniqueID;
			#endif
				const PlayerID sendTo = broadcast ? remoteSystemList[sendList[sendListIndex]].playerId : playerId;
#if RPCID_STRING
				SAMPRakNet::GetCore()->printLn("[SERVER] rpc id=%d to %s bits=%u", rpcLogId, sendTo.ToString(true), bitLength);
#else
				const unsigned short rpcWireId = MapOutgoingLegacyRpcToWire((unsigned char)uniqueID);
				SAMPRakNet::GetCore()->printLn("[SERVER] rpc id=%d wire=%u to %s bits=%u",
					rpcLogId, (unsigned int)rpcWireId, sendTo.ToString(true), bitLength);

				if (((unsigned char)uniqueID == 32 || (unsigned char)uniqueID == 137) && data && bitLength > 0)
				{
					const unsigned int payloadBytes = (bitLength + 7) / 8;
					const unsigned int previewBytes = payloadBytes > 48 ? 48 : payloadBytes;
					static const char hexDigits[] = "0123456789ABCDEF";
					char hexPreview[(48 * 3) + 1];
					unsigned int hexPos = 0;
					for (unsigned int bi = 0; bi < previewBytes && hexPos + 3 < sizeof(hexPreview); ++bi)
					{
						const unsigned char b = ((const unsigned char*)data)[bi];
						hexPreview[hexPos++] = hexDigits[(b >> 4) & 0x0F];
						hexPreview[hexPos++] = hexDigits[b & 0x0F];
						if (bi + 1 < previewBytes)
							hexPreview[hexPos++] = ' ';
					}
					hexPreview[hexPos] = '\0';
					SAMPRakNet::GetCore()->printLn(
						"[SERVER][rpc-out-payload] id=%u wire=%u bytes=%u preview=%s%s",
						(unsigned int)(unsigned char)uniqueID,
						(unsigned int)rpcWireId,
						(unsigned int)payloadBytes,
						previewBytes > 0 ? hexPreview : "<empty>",
...
#if !RPCID_STRING && RAKNET_LEGACY
		// Force exact wire layout used by client bridge->offset path:
		// ID_RPC(8) + rpc_id(9) + WriteCompressed(payloadBits) + payloadBits(raw).
		if (!shiftTimestamp)
		{
			std::vector<unsigned char> compatPacket;
			BuildCompatIdRpcPacket(bridgeRpcWireId, (const unsigned char*)data, bitLength, compatPacket);
			if (!compatPacket.empty())
			{
				const unsigned int currentBytes = outgoingBitStream.GetNumberOfBytesUsed();
				const bool sameSize = currentBytes == (unsigned int)compatPacket.size();
				const bool sameData = sameSize && memcmp(outgoingBitStream.GetData(), &compatPacket[0], compatPacket.size()) == 0;
				if (!sameData)
				{
					outgoingBitStream.Reset();
					outgoingBitStream.Write((const char*)&compatPacket[0], (int)compatPacket.size());
				}
				...
				if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
				{
					const PlayerID sendTo = broadcast ? remoteSystemList[sendList[sendListIndex]].playerId : playerId;
					SAMPRakNet::GetCore()->printLn(
						"[SERVER][rpc-out-final] mode=compat-idrpc same=%u wire=%u payloadBits=%u bytes=%u to %s",
						(unsigned int)(sameData ? 1 : 0),
						(unsigned int)bridgeRpcWireId,
						(unsigned int)bitLength,
						(unsigned int)compatPacket.size(),
						sendTo.ToString(true));
				}
			}
		}
#endif
		...
		{
			const PlayerID sendTo = broadcast ? remoteSystemList[sendList[sendListIndex]].playerId : playerId;
			// Standard RakNet path: send original RPC packet directly.
			Send(&outgoingBitStream, priority, reliability, orderingChannel, sendTo, false);
#if !RPCID_STRING
			// Extra safety for player streaming:
			// duplicate PlayerStreamIn (legacy RPC 32) once, same channel/reliability.
			// This helps clients that occasionally miss the first stream-in edge.
			if (g_bridgeMirrorOriginalRpc && (unsigned char)uniqueID == 32)
			{
				Send(&outgoingBitStream, priority, reliability, orderingChannel, sendTo, false);
				if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
				{
					SAMPRakNet::GetCore()->printLn(
						"[SERVER][rpc-out-dup] id=32 wire=%u to %s bits=%u",
						(unsigned int)MapOutgoingLegacyRpcToWire((unsigned char)uniqueID),
						sendTo.ToString(true),
						(unsigned int)bitLength);
				}
			}
...
{
	// Auth-key enforcement disabled for legacy compatibility:
	// treat missing/invalid auth as regular player unless explicitly NPC.
	if (remoteSystem->sampData.authType == SAMPRakNet::AuthType_Invalid)
	{
	...
	// Legacy client expects: id + challenge(u32) + port(u16) + token(u32), total 11 bytes.
	bitStream.Write(SAMPRakNet::GetToken());
	bitStream.Write(remoteSystem->playerId.port);
	bitStream.Write(SAMPRakNet::GetToken());
	if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
	{
		SAMPRakNet::GetCore()->printLn("[SERVER][hs] id=%u len=%u bits=%u to %s",
			(unsigned int)ID_CONNECTION_REQUEST_ACCEPTED,
			(unsigned int)bitStream.GetNumberOfBytesUsed(),
			(unsigned int)bitStream.GetNumberOfBitsUsed(),
			remoteSystem->playerId.ToString(true));
	}
	...
	// Client handshake expects an extra raw 3-byte marker immediately after accept.
	unsigned char postAcceptMarker[3] = { 0x36, 0x42, 0x52 };
	SendImmediate((char*)postAcceptMarker, sizeof(postAcceptMarker) * 8, SYSTEM_PRIORITY, RELIABLE, 0, remoteSystem->playerId, false, false, RakNet::GetTime());
}
...
bool RakPeer::ParseConnectionAuthPacket(RakPeer::RemoteSystemStruct* remoteSystem, PlayerID playerId, unsigned char* data, int byteSize) {
	...
	if (responseLen < sizeof(auth) && bs.Read(auth, responseLen)) {
		...
		// Auth-key validation is intentionally bypassed.
		// Any non-NPC auth response is treated as a player.
		remoteSystem->sampData.authType = SAMPRakNet::AuthType_Player;
		AcceptConnectionRequest(remoteSystem);
		return true;
	}

	// Even malformed/missing auth response is allowed in compatibility mode.
	remoteSystem->sampData.authType = SAMPRakNet::AuthType_Player;
	AcceptConnectionRequest(remoteSystem);
	return true;
}
...
void RakPeer::OnConnectionRequest( RakPeer::RemoteSystemStruct *remoteSystem, unsigned char *AESKey, bool setAESKey )
{
	...
	//if ( AllowIncomingConnections() )
	{
		// Auth-key challenge/response is disabled; accept connection directly.
		AcceptConnectionRequest(remoteSystem);
...
RakPeer::RemoteSystemStruct * RakPeer::AssignPlayerIDToRemoteSystemList( const PlayerID playerId, RemoteSystemStruct::ConnectMode connectionMode )
{
	...
	//for ( i = 0; i < remoteSystemListSize; i++ )
	{
		if ( remoteSystemList[ i ].isActive==false )
		{
			...
			remoteSystem->reliabilityLayer.SetTimeoutTime((RakNetTime)SAMPRakNet::GetTimeout());
...
bool RakPeer::HandleRPCPacket( const char *data, int length, PlayerID playerId, PlayerIndex playerIndex )
{
...
#else
	unsigned short incomingRpcIdRaw9 = 0;
	{
		// SA-MP client compatibility: RPC ID is serialized on 9 bits on the wire.
		if ( incomingBitStream.ReadBits( (unsigned char*)&incomingRpcIdRaw9, 9, true ) == false )
		{
		...
		unsigned char mappedRpcId = (unsigned char)(incomingRpcIdRaw9 & 0xFF);

		// Prefer candidates that are actually registered on this endpoint.
		unsigned char candidates[32];
		unsigned int candidateCount = 0;
		if (FindIncomingWireCandidates(incomingRpcIdRaw9, candidates, sizeof(candidates), &candidateCount))
		{
			unsigned char fallbackCandidate = candidates[0];
			bool resolvedByRegistration = false;
			for (unsigned int ci = 0; ci < candidateCount && ci < sizeof(candidates); ++ci)
			{
				if (rpcMap.GetIndexFromFunctionName((RPCID)candidates[ci]) != UNDEFINED_RPC_INDEX)
				{
					mappedRpcId = candidates[ci];
					resolvedByRegistration = true;
					break;
				}
			}
			if (resolvedByRegistration == false)
				mappedRpcId = fallbackCandidate;
		}

		uniqueIdentifier = (RPCID)mappedRpcId;

		if (SAMPRakNet::GetCore() && (unsigned short)(unsigned char)uniqueIdentifier != incomingRpcIdRaw9)
		{
			SAMPRakNet::GetCore()->printLn(
				"[CLIENT][rpc-remap] incoming rpc %u -> %u for %s",
				(unsigned int)incomingRpcIdRaw9,
				(unsigned int)(unsigned char)uniqueIdentifier,
				playerId.ToString(true));
		}

		// Legacy client may send PlayerConnect RPC before final handshake state flip.
		// Promote state early so upper layers don't ignore the join.
		if ((unsigned char)uniqueIdentifier == 25)
		{
			RemoteSystemStruct* rs = GetRemoteSystemFromPlayerID(playerId, false, true);
			if (rs && rs->connectMode == RemoteSystemStruct::HANDLING_CONNECTION_REQUEST)
			{
				rs->connectMode = RemoteSystemStruct::CONNECTED;
				rs->connectionTime = RakNet::GetTime();
				if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
				{
					SAMPRakNet::GetCore()->printLn(
						"[CLIENT][hs] promote %s HANDLING_CONNECTION_REQUEST -> CONNECTED on rpc=25",
						playerId.ToString(true));
				}
			}
		}
...
#if RPCID_STRING
		SAMPRakNet::GetCore()->printLn(
			"[CLIENT][rpc-in] id(index)=%d nameEncoded=%d readOffset=%d unreadBits=%d from %s",
			(int)rpcIndex,
			(int)(nameIsEncoded ? 1 : 0),
			incomingBitStream.GetReadOffset(),
			incomingBitStream.GetNumberOfUnreadBits(),
			playerId.ToString(true));
#else
		SAMPRakNet::GetCore()->printLn(
			"[CLIENT][rpc-in] id(raw9)=%u id(mapped8)=%u readOffset=%d unreadBits=%d from %s",
			(unsigned int)incomingRpcIdRaw9,
			(unsigned int)(unsigned char)uniqueIdentifier,
			incomingBitStream.GetReadOffset(),
			incomingBitStream.GetNumberOfUnreadBits(),
	...
	*/
	...
	if (networkIDIsEncoded)
	{
		...
		{
		#if RPCID_STRING
			int rpcLogId = (int)rpcIndex;
		#else
			int rpcLogId = (int)(unsigned char)uniqueIdentifier;
		#endif

			unsigned short payloadRpcId = 0;
			bool hasPayloadRpcId = false;
			if (rpcParms.numberOfBitsOfData >= 16)
			{
				const int prevReadOffset = incomingBitStream.GetReadOffset();
				if (incomingBitStream.ReadBits((unsigned char*)&payloadRpcId, 16, true))
				{
					hasPayloadRpcId = true;
				}
				incomingBitStream.SetReadOffset(prevReadOffset);
			}

			if (hasPayloadRpcId && payloadRpcId >= 300 && payloadRpcId <= 500)
			{
				SAMPRakNet::GetCore()->printLn("[CLIENT] rpc id=%u (payload) from %s bits=%u",
					(unsigned int)payloadRpcId, playerId.ToString(true), rpcParms.numberOfBitsOfData);
			}
			else
			{
				SAMPRakNet::GetCore()->printLn("[CLIENT] rpc id=%d from %s bits=%u",
					rpcLogId, playerId.ToString(true), rpcParms.numberOfBitsOfData);
			}
		}
		...
		if (SAMPRakNet::GetCore() && incomingRpcIdRaw9 == 295)
		{
			...
			const unsigned int previewBytes = payloadBytes > 48 ? 48 : payloadBytes;
			unsigned char payloadPreview[48] = { 0 };
			bool previewRead = true;
			if (previewBytes > 0)
			{
				previewRead = incomingBitStream.ReadBits(payloadPreview, previewBytes * 8, true);
			}
			incomingBitStream.SetReadOffset(prevReadOffset);
			...
			char hexPreview[(48 * 3) + 1];
			unsigned int hexPos = 0;
			for (unsigned int i = 0; i < previewBytes && hexPos + 3 < sizeof(hexPreview); ++i)
			{
				const unsigned char b = payloadPreview[i];
				hexPreview[hexPos++] = hexDigits[(b >> 4) & 0x0F];
				hexPreview[hexPos++] = hexDigits[b & 0x0F];
				if (i + 1 < previewBytes)
					hexPreview[hexPos++] = ' ';
			...
			SAMPRakNet::GetCore()->printLn(
				"[CLIENT][rpc295-payload] from %s bits=%u bytes=%u readOk=%d preview=%s%s",
				playerId.ToString(true),
	...
	if ( rpcParms.numberOfBitsOfData == 0 )
	{
		...
		if (incomingRpcIdRaw9 == 295)
		{
			...
			{
				SAMPRakNet::GetCore()->printLn(
					"[CLIENT][rpc295-fallback] injected nick payload from %s bits=%u",
					playerId.ToString(true),
					(unsigned int)rpcParms.numberOfBitsOfData);
			}
		}
...
//			ShiftIncomingTimestamp( userData, playerId );
		...
		if (incomingRpcIdRaw9 == 295)
		{
			...
			unsigned int nickOffset = 0;

			// Preferred custom payload layout:
			// [0..9] custom fields, [10] nick_len, [11..] nick bytes
			if (payloadBytes >= 12)
			{
				const unsigned int candidateLen = (unsigned int)userData[10];
				if (candidateLen > 0 && candidateLen <= 24 && (11 + candidateLen) <= payloadBytes)
				{
					nickLen = candidateLen;
					nickOffset = 11;
				}
			}

			// Fallback heuristic: find a plausible string length marker.
			if (nickLen == 0 && payloadBytes > 2)
			{
				for (unsigned int i = 0; i + 2 < payloadBytes; ++i)
				{
					const unsigned int candidateLen = (unsigned int)userData[i];
					if (candidateLen == 0 || candidateLen > 24 || (i + 1 + candidateLen) > payloadBytes)
						continue;
					bool printable = true;
					for (unsigned int j = 0; j < candidateLen; ++j)
					{
						const unsigned char ch = userData[i + 1 + j];
			...
			const unsigned char* nickPtr = 0;
			if (nickLen > 0 && nickOffset + nickLen <= payloadBytes)
			{
				nickPtr = userData + nickOffset;
			}
			else
			{
				static const unsigned char kFallbackNick[] = { 'P','l','a','y','e','r' };
				nickPtr = kFallbackNick;
				nickLen = sizeof(kFallbackNick);
			}

			if (nickPtr && nickLen > 0)
			{
				// Build full NetCode::RPC::PlayerConnect payload expected by LegacyNetwork:
				// Version(u32), Modded(u8), Name(str8), Challenge(u32), Key(str8), VersionString(str8), OmpVersion(u32).
				unsigned char normalized[128];
				unsigned int p = 0;
				const auto putU8 = [&](unsigned char v) {
					normalized[p++] = v;
				};
				const auto putU32 = [&](unsigned int v) {
					normalized[p++] = (unsigned char)(v & 0xFFu);
					normalized[p++] = (unsigned char)((v >> 8) & 0xFFu);
					normalized[p++] = (unsigned char)((v >> 16) & 0xFFu);
					normalized[p++] = (unsigned char)((v >> 24) & 0xFFu);
				};
				const auto putStr8 = [&](const unsigned char* s, unsigned int n) {
					putU8((unsigned char)n);
					if (n > 0)
					{
						memcpy(normalized + p, s, n);
						p += n;
					}
				};

				static const unsigned int kLegacyClientVersion037 = 4057;
				const unsigned int challenge = SAMPRakNet::GetToken() ^ kLegacyClientVersion037;
				static const unsigned char kCompatKey[] = { '3', 'E', '9' }; // 0x3E9 = 1001, passes server serial check.
				static const unsigned char kCompatVersion[] = { '0', '.', '3', '.', '7' };

				putU32(kLegacyClientVersion037);
				putU8(1); // Modded
				putStr8(nickPtr, nickLen);
				putU32(challenge);
				putStr8(kCompatKey, sizeof(kCompatKey));
				putStr8(kCompatVersion, sizeof(kCompatVersion));
				putU32(0); // OmpVersion / official-client flag payload

				const unsigned int normalizedLen = p;
				if (normalizedLen <= sizeof(normalized))
				{
					rpcInputOverride = new unsigned char[normalizedLen];
					memcpy(rpcInputOverride, normalized, normalizedLen);
					rpcParms.input = rpcInputOverride;
					rpcParms.numberOfBitsOfData = normalizedLen * 8;

					if (SAMPRakNet::GetCore())
					{
						char nickBuf[32];
						const unsigned int copyLen = nickLen < (sizeof(nickBuf) - 1) ? nickLen : (sizeof(nickBuf) - 1);
						memcpy(nickBuf, nickPtr, copyLen);
						nickBuf[copyLen] = '\0';
						SAMPRakNet::GetCore()->printLn(
							"[CLIENT][rpc295-normalized] nick='%s' bits=%u ver=%u chall=0x%X from %s",
							nickBuf,
							(unsigned int)rpcParms.numberOfBitsOfData,
							(unsigned int)kLegacyClientVersion037,
...
bool RakPeer::SendImmediate( char *data, int numberOfBitsToSend, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool useCallerDataAllocation, RakNetTimeNS currentTime )
{
	...
	{
		if (SAMPRakNet::GetCore() && data && numberOfBitsToSend > 0)
		{
			const PlayerID toPlayer = remoteSystemList[sendList[sendListIndex]].playerId;
			const unsigned int id = numberOfBitsToSend >= 8 ? (unsigned int)(unsigned char)data[0] : 0;
			if (id == (unsigned int)ID_RPC)
			{
				unsigned int rpcWire = 0;
				if (numberOfBitsToSend >= 17 && numberOfBytesUsed >= 3)
				{
					rpcWire = (unsigned int)((unsigned char)data[1]) |
						(unsigned int)((((unsigned char)data[2]) >> 7) & 0x01u) << 8;
				}
				SAMPRakNet::GetCore()->printLn(
					"[SERVER][rpc-out-immediate] wire=%u bits=%u bytes=%u to %s prio=%u rel=%u",
					rpcWire,
					(unsigned int)numberOfBitsToSend,
					(unsigned int)numberOfBytesUsed,
					toPlayer.ToString(true),
					(unsigned int)priority,
					(unsigned int)reliability);
			}
			if (SAMPRakNet::ShouldLogSyncOrRPCPacket((uint8_t)id))
			{
				SAMPRakNet::GetCore()->printLn("[SERVER][pkt-out] id=%u len=%u bits=%u to %s prio=%u rel=%u",
					id, (unsigned int)numberOfBytesUsed, (unsigned int)numberOfBitsToSend, toPlayer.ToString(true),
					(unsigned int)priority, (unsigned int)reliability);
			}
		}
...
namespace RakNet
{
	...
	void __stdcall ProcessNetworkPacket( const unsigned int binaryAddress, const unsigned short port, const char *data, const int length, RakPeer *rakPeer )
		...
		RakPeer::RemoteSystemStruct *remoteSystem;
			...
			playerId.port = port;
			if (SAMPRakNet::GetCore() && data && length > 0 && SAMPRakNet::ShouldLogSyncOrRPCPacket((uint8_t)(unsigned char)data[0]))
			{
				SAMPRakNet::GetCore()->printLn("[CLIENT][pkt-raw] id=%u len=%d bits=%d from %s",
					(unsigned int)(unsigned char)data[0], length, length * 8, playerId.ToString(true));
			}
	...
	//		char data[MAX_OFFLINE_DATA_LENGTH];
		...
		else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_REQUEST && length == sizeof(unsigned char)*3)
		{
			...
			// If we already have an active session from this IP on another port,
			// ignore new open-connection probes to avoid handshake restart loops.
			for (unsigned peerIndex = 0; peerIndex < rakPeer->maximumNumberOfPeers; ++peerIndex)
			{
				RakPeer::RemoteSystemStruct& existing = rakPeer->remoteSystemList[peerIndex];
				if (existing.isActive == false)
					continue;
				if (existing.connectMode != RakPeer::RemoteSystemStruct::CONNECTED)
					continue;
				if (existing.playerId.binaryAddress != playerId.binaryAddress)
					continue;
				if (existing.playerId.port == playerId.port)
					continue;

				SAMPRakNet::GetCore()->printLn(
					"[NET] Ignoring ID_OPEN_CONNECTION_REQUEST from %s while active session exists on %u.%u.%u.%u:%u",
					playerId.ToString(),
					(existing.playerId.binaryAddress & 0xFF),
					((existing.playerId.binaryAddress >> 8) & 0xFF),
					((existing.playerId.binaryAddress >> 16) & 0xFF),
					((existing.playerId.binaryAddress >> 24) & 0xFF),
					(unsigned int)existing.playerId.port);
				return;
			}
			...
			{
				// Reuse an existing in-progress slot from the same IP (different source port),
				// so reconnects do not accumulate UNVERIFIED entries until handshake timeout.
				if (rss == 0)
				{
					RakPeer::RemoteSystemStruct* reusable = 0;
					unsigned reusableIndex = (unsigned)-1;
					for (unsigned peerIndex = 0; peerIndex < rakPeer->maximumNumberOfPeers; ++peerIndex)
					{
						RakPeer::RemoteSystemStruct& existing = rakPeer->remoteSystemList[peerIndex];
						if (existing.isActive == false)
						...
						if (existing.playerId.port == playerId.port)
							continue;
						if (existing.connectMode != RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER &&
							existing.connectMode != RakPeer::RemoteSystemStruct::REQUESTED_CONNECTION &&
							existing.connectMode != RakPeer::RemoteSystemStruct::HANDLING_CONNECTION_REQUEST &&
							existing.connectMode != RakPeer::RemoteSystemStruct::SET_ENCRYPTION_ON_MULTIPLE_16_BYTE_PACKET)
						{
							continue;
						}

						reusable = &existing;
						reusableIndex = peerIndex;
						break;
					}

					if (reusable)
					{
						const PlayerID oldPlayerId = reusable->playerId;
						if (oldPlayerId != playerId)
						{
							rakPeer->remoteSystemLookup.Remove(oldPlayerId);
							reusable->playerId = playerId;

							PlayerIDAndIndex playerIDAndIndex;
							playerIDAndIndex.playerId = playerId;
							playerIDAndIndex.index = reusableIndex;
							rakPeer->remoteSystemLookup.Insert(playerId, playerIDAndIndex);
						}

						reusable->rpcMap.Clear();
						reusable->reliabilityLayer.SetSplitMessageProgressInterval(rakPeer->splitMessageProgressInterval);
						reusable->reliabilityLayer.SetUnreliableTimeout(rakPeer->unreliableTimeout);
						reusable->reliabilityLayer.SetTimeoutTime((RakNetTime)SAMPRakNet::GetTimeout());
						reusable->reliabilityLayer.SetEncryptionKey(0);
						for (unsigned pingIndex = 0; pingIndex < (unsigned)PING_TIMES_ARRAY_SIZE; ++pingIndex)
						{
							reusable->pingAndClockDifferential[pingIndex].pingTime = 65535;
							reusable->pingAndClockDifferential[pingIndex].clockDifferential = 0;
						}

						reusable->connectMode = RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER;
						reusable->pingAndClockDifferentialWriteIndex = 0;
						reusable->lowestPing = 65535;
						reusable->nextPingTime = 0;
						reusable->connectionTime = RakNet::GetTime();
						reusable->weInitiatedTheConnection = false;
						reusable->staticData.Reset();
						reusable->myExternalPlayerId = UNASSIGNED_PLAYER_ID;
						reusable->setAESKey = false;
						reusable->lastReliableSend = reusable->connectionTime;
						reusable->isLogon = false;
						reusable->reliabilityLayer.Reset(true);
						rss = reusable;

						if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
						{
							SAMPRakNet::GetCore()->printLn(
								"[NET] Reusing unverified slot for %s (oldPort=%u newPort=%u)",
								playerId.ToString(),
								(unsigned int)oldPlayerId.port,
								(unsigned int)playerId.port);
						}
					}
				}

				if (rss==0 && rakPeer->GetNumberOfUnverifiedInstances(binaryAddress) > 30)
				{
			...
			else if (rss!=0)
			{
				...
				{
					// Re-acknowledge repeated OPEN_CONNECTION_REQUEST while handshake is in progress.
					// This avoids passing offline packets into the reliability parser on reconnect retries.
					char c[2];
					c[0] = ID_OPEN_CONNECTION_REPLY;
					c[1] = 0; // Pad, some routers apparently block 1 byte packets
					unsigned i;
			...
			// ID_OPEN_CONNECTION_REQUEST is an offline packet and must not be forwarded to reliability processing.
			return;
		}

		// See if this datagram came from a connected system
		const unsigned char incomingId = (unsigned char)data[0];
		const bool looksLikeAckDatagram =
			(length <= 5) &&
			(incomingId == 0xE2 || incomingId == 0xE3 || incomingId == 0xE4 || incomingId == 0xE6 || incomingId == 0xC2 || incomingId == 0xC4);
		if (looksLikeAckDatagram && SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
		{
			SAMPRakNet::GetCore()->printLn(
				"[NET][ack-wire] recv id=0x%02X len=%d from %s",
		...
		if (remoteSystem == 0)
		{
			// Generic NAT port-migration guard:
			// if a peer changes UDP source port mid-session, rebind by IP when there is
			// exactly one active/verified candidate for this IP.
			RakPeer::RemoteSystemStruct* preferredCandidate = 0;
			unsigned preferredIndex = (unsigned)-1;
			bool ambiguousPreferred = false;
			for (unsigned peerIndex = 0; peerIndex < rakPeer->maximumNumberOfPeers; ++peerIndex)
			{
				RakPeer::RemoteSystemStruct& existing = rakPeer->remoteSystemList[peerIndex];
				if (existing.isActive == false)
				...
				if (existing.playerId.binaryAddress != playerId.binaryAddress)
					continue;
				if (existing.connectMode == RakPeer::RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY)
					continue;
				if (existing.connectMode != RakPeer::RemoteSystemStruct::CONNECTED &&
					existing.connectMode != RakPeer::RemoteSystemStruct::HANDLING_CONNECTION_REQUEST &&
					existing.connectMode != RakPeer::RemoteSystemStruct::SET_ENCRYPTION_ON_MULTIPLE_16_BYTE_PACKET)
				...
				{
					ambiguousPreferred = true;
					break;
				}
				preferredCandidate = &existing;
				preferredIndex = peerIndex;
			}

			if (preferredCandidate && ambiguousPreferred == false && preferredCandidate->playerId.port != playerId.port)
			{
				const PlayerID oldPlayerId = preferredCandidate->playerId;
				rakPeer->remoteSystemLookup.Remove(oldPlayerId);
				preferredCandidate->playerId = playerId;

				PlayerIDAndIndex playerIDAndIndex;
				playerIDAndIndex.playerId = playerId;
				playerIDAndIndex.index = preferredIndex;
				rakPeer->remoteSystemLookup.Insert(playerId, playerIDAndIndex);
				remoteSystem = preferredCandidate;

				if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
				{
					SAMPRakNet::GetCore()->printLn(
						"[NET][port-migrate] rebound %s oldPort=%u newPort=%u rs=%p mode=%d",
						playerId.ToString(),
						(unsigned int)oldPlayerId.port,
						(unsigned int)playerId.port,
						preferredCandidate,
						(int)preferredCandidate->connectMode);
				}
			}
		}
		if (remoteSystem == 0 && looksLikeAckDatagram)
		{
			// First, if this is a single-message ACK datagram, try to bind by ACK content.
			// This resolves cases where multiple remote systems share the same IP.
			if (incomingId == 0xE3 && length == 3)
			{
				const MessageNumberType ackMessage =
					(MessageNumberType)(((unsigned char)data[1]) | (((unsigned char)data[2]) << 8));
				RakPeer::RemoteSystemStruct* matchedByAck = 0;
				unsigned matchedByAckIndex = (unsigned)-1;
				bool ambiguousByAck = false;
				...
				for (unsigned peerIndex = 0; peerIndex < rakPeer->maximumNumberOfPeers; ++peerIndex)
				{
					...
					if (existing.connectMode == RakPeer::RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY)
						continue;
					if (existing.reliabilityLayer.HasUnacknowledgedMessage(ackMessage) == false)
						continue;
					...
					matchedByAck = &existing;
					matchedByAckIndex = peerIndex;
				}

				if (matchedByAck && ambiguousByAck == false)
				{
					const PlayerID oldPlayerId = matchedByAck->playerId;
					if (oldPlayerId.port != playerId.port)
					{
						rakPeer->remoteSystemLookup.Remove(oldPlayerId);
						matchedByAck->playerId = playerId;

						PlayerIDAndIndex playerIDAndIndex;
						playerIDAndIndex.playerId = playerId;
						playerIDAndIndex.index = matchedByAckIndex;
						rakPeer->remoteSystemLookup.Insert(playerId, playerIDAndIndex);

						if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
						{
							SAMPRakNet::GetCore()->printLn(
								"[NET][ack-wire] rebound-by-ack %s oldPort=%u newPort=%u msg=%u rs=%p mode=%d",
								playerId.ToString(),
								(unsigned int)oldPlayerId.port,
								(unsigned int)playerId.port,
								(unsigned int)ackMessage,
								matchedByAck,
								(int)matchedByAck->connectMode);
						}
					}

					remoteSystem = matchedByAck;
				}
				else if (ambiguousByAck && SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
				{
					SAMPRakNet::GetCore()->printLn(
						"[NET][ack-wire] ambiguous ack-match for %s msg=%u, skip rebound",
						playerId.ToString(),
						(unsigned int)ackMessage);
				}
			}

			// Fallback: NAT port-rebinding by IP-only when unambiguous.
			if (remoteSystem == 0)
			{
			// NAT port-rebinding guard:
			// If ACK comes from same IP but different source port, try to rebind only when
			// we can unambiguously identify exactly one active remote system on that IP.
			RakPeer::RemoteSystemStruct* reboundCandidate = 0;
			unsigned reboundIndex = (unsigned)-1;
			...
			for (unsigned peerIndex = 0; peerIndex < rakPeer->maximumNumberOfPeers; ++peerIndex)
			{
				...
				if (reboundCandidate == 0)
				{
					reboundCandidate = &existing;
					reboundIndex = peerIndex;
				}
				else
				{
				...
				// Prefer verified/handshake-active slots over UNVERIFIED ones.
				const bool isPreferredMode =
					existing.connectMode == RakPeer::RemoteSystemStruct::CONNECTED ||
					existing.connectMode == RakPeer::RemoteSystemStruct::HANDLING_CONNECTION_REQUEST ||
					existing.connectMode == RakPeer::RemoteSystemStruct::SET_ENCRYPTION_ON_MULTIPLE_16_BYTE_PACKET;
				if (isPreferredMode)
				{
			...
			// If exactly one preferred candidate exists, use it even if generic IP-match is ambiguous.
			if (preferredCandidate && ambiguousPreferred == false)
			{
				reboundCandidate = preferredCandidate;
				reboundIndex = preferredIndex;
				ambiguousCandidate = false;
			}

			if (reboundCandidate && ambiguousCandidate == false)
			{
				const PlayerID oldPlayerId = reboundCandidate->playerId;
				if (oldPlayerId.port != playerId.port)
				{
					rakPeer->remoteSystemLookup.Remove(oldPlayerId);
					reboundCandidate->playerId = playerId;

					PlayerIDAndIndex playerIDAndIndex;
					playerIDAndIndex.playerId = playerId;
					playerIDAndIndex.index = reboundIndex;
					rakPeer->remoteSystemLookup.Insert(playerId, playerIDAndIndex);

					if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
					{
						SAMPRakNet::GetCore()->printLn(
							"[NET][ack-wire] rebound %s oldPort=%u newPort=%u rs=%p mode=%d",
							playerId.ToString(),
							(unsigned int)oldPlayerId.port,
							(unsigned int)playerId.port,
							reboundCandidate,
							(int)reboundCandidate->connectMode);
					}
				}
				remoteSystem = reboundCandidate;
			}
			else if (ambiguousCandidate && SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
			{
				SAMPRakNet::GetCore()->printLn(
					"[NET][ack-wire] ambiguous ip-match for %s, skip rebound",
					playerId.ToString());
			}
			}
		}
		...
		{
			SAMPRakNet::GetCore()->printLn(
				"[NET][ack-wire] route from %s rs=%p mode=%d",
				playerId.ToString(true),
				remoteSystem,
				remoteSystem ? (int)remoteSystem->connectMode : -1);
		}
		if ( remoteSystem )
		{
			bool shouldBanPeer = false;
			// Client logic (from decompiled ProcessNetworkPacket) arms encryption when
			// connectMode==SET_ENCRYPTION_ON_MULTIPLE_16_BYTE_PACKET and packet length is
			// divisible by 8. Mirror that behavior for compatibility.
			...
			{
				// Temporary protection mode: ignore disconnect triggers produced by
				// malformed traffic so active players stay connected.
				if (!SAMPRakNet::IsTempConnectionProtectionEnabled())
		...
		else
		{
			...
			{
				SAMPRakNet::GetCore()->printLn(
					"[NET][ack-wire] drop no-remote-system from %s id=0x%02X len=%d",
					playerId.ToString(true),
					(unsigned int)incomingId,
					length);
			}
	...
	//	unsigned numberOfBytesUsed;
		...
		while (rcs)
		{
			...
			if (rcs->nextRequestTime < timeMS)
			{
				...
				unsigned i;
				const unsigned char rawOpenConnectionProbe[3] = { 0x36, 0x24, 0x42 };
				for (i=0; i < messageHandlerList.Size(); i++)
					messageHandlerList[i]->OnDirectSocketSend((char*)rawOpenConnectionProbe, 24, rcs->playerId);
				SocketLayer::Instance()->SendTo( connectionSocket, (const char*)rawOpenConnectionProbe, 3, rcs->playerId.binaryAddress, rcs->playerId.port );
				char c[3];
		...
		//	remoteSystemList[ remoteSystemIndex ].allowPlayerIdAssigment=true;
			if ( remoteSystemList[ remoteSystemIndex ].isActive )
			{
				...
				const RakNetTime configuredTimeout = (RakNetTime)SAMPRakNet::GetTimeout();
				if (remoteSystem->reliabilityLayer.GetTimeoutTime() != configuredTimeout)
					remoteSystem->reliabilityLayer.SetTimeoutTime(configuredTimeout);
				...
				if (!LEGACY_KEEP_CONNECTED_WITHOUT_ACK &&
					timeMS > remoteSystem->lastReliableSend &&
					timeMS-remoteSystem->lastReliableSend > 5000 &&
				...
				bool deadConnectionDetected = remoteSystem->reliabilityLayer.IsDeadConnection();
				if (LEGACY_KEEP_CONNECTED_WITHOUT_ACK &&
					remoteSystem->connectMode==RemoteSystemStruct::CONNECTED)
				{
					deadConnectionDetected = false;
				}
				const bool disconnectAsapDrained =
					(remoteSystem->connectMode==RemoteSystemStruct::DISCONNECT_ASAP ||
					remoteSystem->connectMode==RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY) &&
					remoteSystem->reliabilityLayer.IsDataWaiting()==false;
				const bool disconnectOnNoAckDone =
					remoteSystem->connectMode==RemoteSystemStruct::DISCONNECT_ON_NO_ACK &&
					remoteSystem->reliabilityLayer.AreAcksWaiting()==false;
				const bool handshakeTimeout =
					...
					timeMS - remoteSystem->connectionTime > LEGACY_HANDSHAKE_TIMEOUT_MS;

				if ( deadConnectionDetected || disconnectAsapDrained || disconnectOnNoAckDone || handshakeTimeout )
				{
					if (SAMPRakNet::IsTempConnectionProtectionEnabled() &&
						remoteSystem->connectMode == RemoteSystemStruct::CONNECTED)
					{
					...
					if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
					{
						RakNetStatisticsStruct *dropStats = remoteSystem->reliabilityLayer.GetStatistics();
						SAMPRakNet::GetCore()->printLn(
							"[SERVER][drop] peer=%s mode=%d reasons(dead=%d asapDrained=%d noAckDone=%d hsTimeout=%d) ms(now=%u conn=%u lastRel=%u) resend=%u acksPending=%u dataWaiting=%d",
							playerId.ToString(true),
							(int)remoteSystem->connectMode,
							deadConnectionDetected ? 1 : 0,
							disconnectAsapDrained ? 1 : 0,
							disconnectOnNoAckDone ? 1 : 0,
							handshakeTimeout ? 1 : 0,
							(unsigned int)timeMS,
							(unsigned int)remoteSystem->connectionTime,
							(unsigned int)remoteSystem->lastReliableSend,
							dropStats ? (unsigned int)dropStats->messagesOnResendQueue : 0u,
							dropStats ? (unsigned int)dropStats->acknowlegementsPending : 0u,
							remoteSystem->reliabilityLayer.IsDataWaiting() ? 1 : 0);
					}
					...
					// For unknown senders we only accept a few specific packets
						if (remoteSystem->connectMode==RemoteSystemStruct::UNVERIFIED_SENDER)
						{
							...
							else if ((unsigned char)(data)[0] == ID_RPC)
							{
								...
								if (byteSize >= 3)
								{
									RakNet::BitStream rpcProbe((unsigned char*)data, byteSize, false);
									unsigned char packetId = 0;
									if (rpcProbe.Read(packetId) && packetId == ID_RPC &&
										rpcProbe.ReadBits((unsigned char*)&rpcId9, 9, true))
									{
										parsedRpc = true;
									}
								}
								...
								if (parsedRpc && rpcId9 == 295 && remoteSystem->sampData.unverifiedRPCs++ < MAX_UNVERIFIED_RPCS)
								{
									...
									HandleRPCPacket((char*)data, byteSize, playerId, (PlayerIndex)remoteSystemIndex);
									delete[] data;
								}
								else
								{
									// Keep socket alive during pre-auth chatter; ignore unmatched RPCs.
									delete[] data;
								}
							}
							else
							{
								// Pre-handshake compatibility mode: ignore unknown pre-auth packets.
								if (data)
									delete[] data;
							}
						}
	...
	//						RakAssert(remoteSystem->connectMode==RemoteSystemStruct::HANDLING_CONNECTION_REQUEST);
						...
						{
							const bool protectionEnabled = SAMPRakNet::IsTempConnectionProtectionEnabled();
							if (SAMPRakNet::GetCore())
							{
								SAMPRakNet::GetCore()->printLn(
									"[NET][disconnect-wire] recv id=18(ID_DISCONNECTION_NOTIFICATION) from %s mode=%d protection=%d",
									playerId.ToString(true),
									(int)remoteSystem->connectMode,
									protectionEnabled ? 1 : 0);
							}

							// Temporary protection mode: ignore remote disconnect notification.
						...
						{
							// Late duplicate from legacy clients: we've already transitioned.
							if (remoteSystem->connectMode==RemoteSystemStruct::CONNECTED)
							{
								if (data)
									delete [] data;
							...
							//	playerId.port = remotePort;
								...
								// Legacy compact accept packet does not include external ID.
								remoteSystem->myExternalPlayerId = playerId;
								remoteSystem->connectMode=RemoteSystemStruct::CONNECTED;
							...
							{
								// Tell the remote system the connection failed (unless temporary protection mode is enabled).
								if (!SAMPRakNet::IsTempConnectionProtectionEnabled())
									NotifyAndFlagForDisconnect(playerId, true, 0);
	#ifdef _DO_PRINTF
						...
						else if (data[0] == (unsigned char)ID_CONNECTION_LOST)
						{
							...
							{
								SAMPRakNet::GetCore()->printLn(
									"[NET][disconnect-wire] recv id=17(ID_CONNECTION_LOST) from %s mode=%d (ignored)",
									playerId.ToString(true),
									(int)remoteSystem->connectMode);
							}
							delete[] data;
						...
						{
							const unsigned char packetId = (unsigned char)data[0];
							if (SAMPRakNet::GetCore() && SAMPRakNet::ShouldLogSyncOrRPCPacket(packetId))
							{
								SAMPRakNet::GetCore()->printLn(
									"[CLIENT] packet id=%u bits=%u from %s",
